...
class PluginActions extends React.Component {
  openGitHub(event, name) {
    ...
    shell.openExternal(
      `https://github.com/kko7/league-client-enhancer/issues/new?labels=bug&title=${name} bug`
    )
  }
  ...
  render() {
    return (
      <CardActions>
        <CardActionButtons>
          <CardActionButton
            onClick={(event) => this.disablePlugin(event, this.props.id)}
          >
            {this.props.enabled ? "Disable" : "Enable"}
          </CardActionButton>
          <CardActionButton
            onClick={(event) => this.createWindow(event, this.props.name)}
          >
            Settings
          </CardActionButton>
        </CardActionButtons>
        <CardActionIcons>
          <CardActionIcon
            onClick={(event) => this.openGitHub(event, this.props.name)}
            icon="announcement"
          />
        </CardActionIcons>
      </CardActions>
